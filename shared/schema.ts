...
export const syncStatusEnum = pgEnum("sync_status", ["queued", "running", "success", "error"]);
...
export const projects = pgTable("projects", {
  ...
  features: json("features").$type<string[]>().default(sql`'[]'`),
  images: json("images").$type<Array<{ url: string; alt: string }>>().default(sql`'[]'`),
  topics: json("topics").$type<string[]>().default(sql`'[]'`),
  languages: json("languages").$type<Record<string, number>>().default(sql`'{}'`),
